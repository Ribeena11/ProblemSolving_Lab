...
class Song:
    ...
    def __str__(self): # ต้องการให้แสดงรายละเอียดทั้งหมด
        return f"{self.title} by {self.artist}"

class MusicPlaylist:
    def __init__(self):
        self.head = None  # The head of the linked list (first song) **จำเป็นต้องทำ เพราะยังไม่รู้ข้อมูล**
        self.current_song = None # To keep track of the currently playing song
        self.length = 0 # How many song in play list

    def add_song(self, title, artist, songfile): # ต้องสัมพันธ์กับ class Song ที่สร้างไว้
        new_song = Song(title, artist, songfile)
        if self.head is None: # ยังไม่มีโหนดแรก
            self.head = new_song
    ...
    def prev_song(self):
        if self.head is None or self.current_song is None:
            ...
            return
        if self.current_song == self.head: # กด prev แล้วตรงกับ head พอดี
